...
function KasPenuntunScreen({ user, cabang, cabangList, onBack, onLogout }) {
  ...
  const penuntunRows = (() => {
    ...
    // Collect all dates in selected month
    ...
    // Calculate tunai pasar per date (same logic as BukuRekapScreen, across all resorts)
    ...
    sortedDates.forEach(dateStr => {
      ...
      // Saldo Kas = carry-forward dari hari sebelumnya + drop pusat hari ini
  ...
  const tdC = { padding: '5px 6px', textAlign: 'center', fontSize: 11, borderRight: '1px solid var(--border-light)', fontWeight: 600 };
            ...
            <table style={{ width: '100%', minWidth: 1000, fontSize: 12, borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {penuntunRows.map((row) => (
                  <>
                    {/* Sub-row 1: Tunai Pasar Hari Ini */}
